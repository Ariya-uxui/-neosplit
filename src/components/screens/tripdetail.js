...
function TripDetail({ setPage, tripBills, deleteExpense, startEditExpense }) {
  const total = tripBills.reduce((sum, bill) => sum + bill.amount, 0);
  ...
  return (
    <div className="screen-green">
      <div className="top-pill-green">Bills Details</div>
        ...
        {tripBills.length === 0 ? (
          <div className="white-panel center mt-16">
            ...
            <div style={{ fontSize: 14 }}>Add your first expense to get started</div>
          </div>
        ) : (
          tripBills.map((bill) => (
              ...
              <div className="trip-bill-left">
                ...
                <div>
                  ...
                  <div className="trip-bill-meta">
                    Paid by {bill.paidBy} • {bill.pax} pax
                  </div>
                </div>
              </div>
      ...
      <button className="btn-gray mt-16" onClick={() => setPage("addexpense")}>
        + Add Expense
      </button>

      <button className="btn-black mt-12" onClick={() => setPage("settlement")}>
        Settlement
      </button>
    </div>
  );
