    ...
    <div className="screen-green leaderboard-simple-screen">
      ...
      <div className="leaderboard-simple-list">
        {users.map((user, index) => (
          <div key={index} className="leader-row-simple">
            <div className="leader-row-left-simple">
              <span className="leader-badge-simple">{user.badge}</span>
              <div>
                <div className="leader-name-simple">{user.name}</div>
                <div className="leader-rank-simple">Rank #{index + 1}</div>
              </div>
            </div>

            <div className="leader-points-simple">{user.points} pts</div>
          </div>
        ))}
      </div>

      <button className="btn-black mt-20" onClick={() => setPage("mypoints")}>
        My points
      </button>
    </div>
  );
